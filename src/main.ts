  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      case '--target':
        ...
        break;
      case '--port':
        result.port = parseIntegerFlag('--port', next);
        i++;
        break;
      case '--https-port':
        result.httpsPort = parseIntegerFlag('--https-port', next);
        i++;
        break;
      case '--password':
        result.dashboardPassword = next;
        i++;
        break;
      case '--cert':
        result.tls = { ...result.tls, cert: next };
        i++;
        break;
      case '--key':
        result.tls = { ...result.tls, key: next };
        i++;
        break;
      case '--self-signed':
        result.tls = { ...result.tls, selfSigned: true };
        break;
      case '--config':
        result.configFile = next;
        i++;
        break;
      case '--dstat':
        result.dstat = true;
        break;
      case '--dstat-only':
        result.dstatOnly = true;
        result.dstatUrl = next ?? 'http://localhost:8080';
        i++;
        break;
      case '--dstat-refresh':
        result.dstatRefresh = parseIntegerFlag('--dstat-refresh', next) * 1000;
        i++;
        break;
      case '--help':
        printHelp();
...
function getParsedArgs() {
  return (parsedArgs ??= parseArgs());
}

function loadConfig(): { config: ServerConfig; auditLines: string[] } {
  ...
  const compiled = compileServerConfig({
    fileConfig,
    cliOverrides: cli,
  });

  return {
    config: compiled.config as ServerConfig,
    auditLines: renderSecurityAudit(compiled.audit),
  };
}
...
function main(): void {
  ...
  try {
    ...
    log.error(error instanceof Error ? error.message : 'Failed to parse CLI arguments');
    process.exit(1);
  }
  ...
  try {
    ...
    auditLines = loaded.auditLines;
  } catch (error) {
    log.error(error instanceof Error ? error.message : 'Invalid configuration');
    process.exit(1);
  }
